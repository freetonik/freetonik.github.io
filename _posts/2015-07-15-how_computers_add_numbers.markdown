---
layout: post
title: "Как компьютеры складывают числа"
category: Translations
original_url: http://habrahabr.ru/company/hexlet/blog/262871/
original_url_source: Habrahabr
original_url_trans: https://www.youtube.com/watch?v=VBDoT8o4q00
original_author_trans: In One Lesson
minutes: 15
video_id: YuSgZ173Utg
video_provider: youtube
---

Мы в <a href="https://ru.hexlet.io/?utm_medium=blog&utm_source=rakhim&utm_campaign=translation">Хекслете</a> любим разрабатывать не только прикладные курсы, но и более фундаментальные (например, про алгоритмы или операционные системы). Но мы пока не спускались ниже уровня ОС в иерархии абстракций. А там, внутри, столько всего интересного! Для многих людей, да даже для многих профессиональных программистов остаются загадкой процессы, происходящие внутри микропроцессора, на уровне отдельных транзисторов.

Публикуем перевод замечательного видео, в котором меньше чем за 15 минут объясняется, как компьютеры складывают числа с помощью транзисторов, двоичной системы счисления, простых логических схем и их хитрых комбинаций.

<div class="video-container mb3"><iframe width="560" height="420" frameborder="0" allowfullscreen src="https://www.youtube.com/embed/YuSgZ173Utg?color=white&theme=light&rel=0&amp;showinfo=0"></iframe></div>

Перевод Наталии Басс, озвучка моя.

*P.S. В начале видео ошибочно процессором называют северный мост.*
